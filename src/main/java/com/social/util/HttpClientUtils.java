...
public class HttpClientUtils {
    ...
    public static String sendGetRequest(String reqURL, String decodeCharset) {
        ...
        try {
            ...
            log.info("HttpClientUtils#sendGetRequest ->  request address: " + httpGet.getURI());
            log.info("HttpClientUtils#sendGetRequest ->  response status: " + response.getStatusLine());
            log.info("HttpClientUtils#sendGetRequest ->  response length: " + responseLength);
            log.info("HttpClientUtils#sendGetRequest -> response content: " + responseContent);
        } catch (ClientProtocolException e) {
            log.error("HttpClientUtils#sendGetRequest -> The exception is usually the result of a protocol error,For example, the introduction of the "
            ...
            log.error("HttpClientUtils#sendGetRequest -> The stack information is as follows:" + Arrays.toString(e.getStackTrace()));
        } catch (IOException e) {
            log.error("HttpClientUtils#sendGetRequest -> The exception is usually caused by the network causes, such as the HTTP server is not started, the stack information is as follows: ");
            log.error(e.getStackTrace());
        } finally {
    ...
    public static String sendRequest(String url) throws Exception {
        ...
        try {
            ...
            log.info("HttpClientUtils#sendRequest -> Request URL:" + url);
            urlConnection.connect();
            inPutStream = urlConnection.getInputStream();

            int size = inPutStream.available();
            byte[] jsonBytes = new byte[size];
            int read = inPutStream.read(jsonBytes);
            response = new String(jsonBytes, "UTF-8");
            log.info("HttpClientUtils#sendGetRequest -> response string:" + response);
        } finally {
