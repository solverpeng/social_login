...
public class SocialCallBackController extends BaseController {
    ...
    public void qq() throws Exception {
        ...
        log.info("SocialCallBackController#qq -> accessToken：" + accessToken);
        log.info("SocialCallBackController#qq ->      openId：" + openId);
    ...
    public void weChat() throws Exception { // web and mobileweb common method
        ...
        if (state == null || !state.equals(sessionState)) {
            log.error("SocialCallBackController#wechat -> State is no init.");
        }
        JSONObject result = WeChatDevUtils.oauth(code);
        log.info("SocialCallBackController#wechat -> get accessToken result:" + result.toString());
    ...
    private void sinaWeiboLoginAction(String code) throws WeiboException, IOException {
        ...
        log.info("SocialCallBackController#sinaWeiboLoginAction -> weibo accessToken:" + accessToken.getAccessToken() + " uid:" + uid);
        ...
        if (userVO == null) {
            ...
            checkLogin(uid, OpenIdType.WEIBO, screenName);
        }
    }
    ...
    private void getQQUserInfoByAccessToken(String accessToken, String openID) throws Exception {
        ...
        checkLogin(openID, OpenIdType.QQ, jsonUserInfo.getString("nickname"));
    }
    ...
    private void wechatResultJson() throws Exception {
        if (getRequest().getSession().getAttribute("oauth_name") == null || getRequest().getSession().getAttribute("oauth_access_token") == null ||
            ...
            log.error("SocialCallBackController#wechatResultJson -> Access Token has expired or does not exist!");
            return;
        }
        ...
        if (!chkAccessToken(accessToken, openId)) {
            ...
            log.info("SocialCallBackController#wechatResultJson -> refreshToken:" + jsonResult.toString());
            accessToken = jsonResult.getString("access_token");
        }

        WxUserInfo userInfo = WeChatDevUtils.getUserInfoBySns(accessToken, openId);
        if (userInfo == null) {
            log.error("SocialCallBackController#wechatResultJson -> unable to obtain authorization user information!");
            return;
        }

        log.info("SocialCallBackController#wechatResultJson ->     userInfo:" + userInfo);
        log.info("SocialCallBackController#wechatResultJson ->    oauthName:" + oauthName);
        log.info("SocialCallBackController#wechatResultJson ->  accessToken:" + accessToken);
        log.info("SocialCallBackController#wechatResultJson ->       openId:" + openId);

        checkLogin(openId, OpenIdType.WECHAT, userInfo.getNickname());
    }
    ...
    private boolean chkAccessToken(String accessToken, String openId) throws Exception {
        ...
        log.info("SocialCallBackController#chkAccessToken -> jsonResult:" + jsonResult.toString());
    ...
    public void checkLogin(String openId, OpenIdType openIdType, String name) throws IOException {
